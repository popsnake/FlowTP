...
def main(argv=None):
    parser = argparse.ArgumentParser(description='FlowTP generation and original task-head evaluation')
    parser.add_argument('--task', choices=['amp', 'afp', 'avp'], default='afp')
    parser.add_argument('--count', type=int, default=1000)
    parser.add_argument('--input', help='Evaluate an existing FASTA instead of generating')
    parser.add_argument('--output', help='Save generated (or evaluated) sequences as FASTA')
    parser.add_argument('--generate-only', action='store_true')
    args = parser.parse_args(argv)
    sample_type = args.task
    assert sample_type.lower() in ['amp', 'afp', 'avp']

    cfs = 1.5
    n = 1
    sample_num = args.count
    ...
    def flow_matching_sample(flow_matcher, x_shape, num_steps=200, len_list=None, use_attention=True,
        ...
        if len_list is None:
            length_sampler = LengthSampler(
                path=f'data/split_v2/{peptide_file}.fasta',
    ...
    flow_matcher.load_state_dict(torch.load("save_model/flowy_matcher_model.pkl", map_location=device))

    tokenizer = AutoTokenizer.from_pretrained(conf_dict['denoiser_esm_model_name'], trust_remote_code=True)
    esm2_model = AutoModelForMaskedLM.from_pretrained(conf_dict['denoiser_esm_model_name'], trust_remote_code=True).to(device)
    ...
    if sample_type == 'amp':
        ...
        decoder.load_state_dict(torch.load("save_model/antimicrobial_decoder_model_1.pkl", map_location=device))
    elif sample_type == 'afp':
        peptide_file = 'antifungal'
        decoder.load_state_dict(torch.load("save_model/antifungal_decoder_model_1.pkl", map_location=device))
    else:
        peptide_file = 'antiviral'
        decoder.load_state_dict(torch.load("save_model/antiviral_decoder_model_1.pkl", map_location=device))
    ...
    missing_amino_acids = [aa for aa in canonical_amino_acids if aa not in tokenizer.get_vocab()]
    if missing_amino_acids:
        raise ValueError(f"Tokenizer is missing canonical amino acids: {missing_amino_acids}")
    canonical_id_to_aa = {
        tokenizer.convert_tokens_to_ids(aa): aa for aa in canonical_amino_acids
    }
    min_length = int(conf_dict['min_length'])
    max_length = int(conf_dict['max_length'])

    with open('data/split_v2/train.csv', newline='', encoding='utf-8-sig') as train_file:
        training_sequences = {
            row['sequence'].strip().upper()[:max_length]
    ...
    def decode_and_filter(seq_ids, seen_set):
        ...
        residue_ids = [int(token_id) for token_id in seq_ids[1:eos_index].tolist()]
        if any(token_id not in canonical_id_to_aa for token_id in residue_ids):
            return None, "noncanonical"
        if len(residue_ids) < min_length:
            return None, "too_short"

        seq = ''.join(canonical_id_to_aa[token_id] for token_id in residue_ids)
        if seq in training_sequences:
            return None, "training_duplicate"
        if seq in seen_set:
            return None, "generated_duplicate"
    ...
    if args.input is None:
        ...
        seq_list = [str(record.seq) for record in SeqIO.parse(args.input, 'fasta')]

    if args.output:
        Path(args.output).parent.mkdir(parents=True, exist_ok=True)
        Path(args.output).write_text(''.join(f'>sequence_{i}\n{s}\n' for i, s in enumerate(seq_list, 1)), encoding='utf-8')
    ...
    real_file = "data/split_v2/all_training.fasta"
    similarity_scores = []
